...
class RateLimiter {
  constructor() {
    ...
    this.blockedIPs = new Set(); // Cache für geblockte IPs
    this.lastBlockedIPsUpdate = 0;
    
    // Cleanup alte Entries alle 5 Minuten
    setInterval(() => this.cleanup(), 5 * 60 * 1000);
  }

  // Lade geblockte IPs aus DB (mit Cache)
  async loadBlockedIPs() {
    const now = Date.now();
    // Cache für 1 Minute
    if (now - this.lastBlockedIPsUpdate < 60000 && this.blockedIPs.size > 0) {
    ...
    try {
      ...
      const blocked = await sql`
        SELECT ip_address FROM blocked_ips WHERE is_active = true
      `;
      
      this.blockedIPs = new Set(blocked.map(row => row.ip_address));
      this.lastBlockedIPsUpdate = now;
    } catch (error) {
