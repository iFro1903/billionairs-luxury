...
class StripePaymentProcessor {
    ...
    async createSepaCheckoutSession() {
        if (this.isProcessing) {
            console.log('Payment already in progress...');
            return;
        }
        ...
        if (!emailInput || !nameInput) {
            this.showPaymentError('Please fill in your email and name');
            return;
        }
        ...
        try {
            ...
            console.log('🏦 Starting SEPA payment process:', { email, name });

            const response = await fetch('/api/stripe-sepa', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            console.log('📡 SEPA Response status:', response.status);

            if (!response.ok) {
                const errorData = await response.json();
                console.error('❌ SEPA Server error:', errorData);
                throw new Error(errorData.error || 'SEPA payment failed');
            }

            const session = await response.json();
            console.log('✅ SEPA Session created:', session);

            if (!session.url) {
                throw new Error('No SEPA checkout URL received');
            }
